...
class WeixinBaseGateway extends GateWay
{
    ...
    public static function getInstance()
    {
        ...
        $merchantPrivateKeyPath = self::$config['merchantPrivateKeyPath'];
        $merchantPrivateKeyInstance = Rsa::from($merchantPrivateKeyPath, Rsa::KEY_TYPE_PRIVATE);

        // 「商户API证书」的「证书序列号」
        $merchantCertificateSerial = self::$config['merchantSerial'];

        // 从本地文件中加载「微信支付平台证书」，用来验证微信支付应答的签名
        $platformCertificatePath = self::$config['platformCertificatePath'];
        $platformPublicKeyInstance = Rsa::from($platformCertificatePath, Rsa::KEY_TYPE_PUBLIC);

        // 从「微信支付平台证书」中获取「证书序列号」
        $platformCertificateSerial = PemUtil::parseCertificateSerialNo($platformCertificatePath);
