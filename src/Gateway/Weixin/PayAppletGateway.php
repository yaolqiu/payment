...
class PayWeixinGateway extends WeixinBaseGateway implements GateWayInterface
{
    ...
    public function request(array $param)
    {
        ...
        $data = [
            ...
            'callback_url'=> $param['notify_url'] ?? '',        //C 前端页面跳转地址，支付完成后将回跳该地址，不能携带参数
            'terminal_ip'=> $param['terminal_ip'] ?? '',        //C 终端IP H5-微信支付时必填，客户端访问IP
            'terminal_device'=>$param['terminal_device'] ?? '4',//C	终端设备 1:PC浏览器  2:WEB浏览器  3:微信公众号 4:微信小程序 5:手机APP  6:其它
            'bank_id'=> $param['bank_id'] ?? '4',               //C 银行编码
            'auth_app_id'=> $param['auth_app_id'] ?? '',        //C 应用APPID 微信或支付宝应用APPID （公众号和小程序支付必传应用APPID）
            'openid'=> $param['openid'] ?? '',                  //C 微信用户标识
            'is_raw'=> $param['is_raw'] ?? '',                  //C 是否原生支付
            'buyer_id'=> $param['buyer_id'] ?? '',              //C 支付宝用户标识
            'auth_code'=> $param['auth_code'] ?? '',            //C 支付授权码
            ...
            'expire_date'=> $param['expire_date'] ?? '',  //C 订单支付有效期时间
            'time_expire'=> $param['time_expire'] ?? '',  //C 该笔订单允许的最晚付款时间
            'memo'=> $param['memo'] ?? '' //C 备注说明
        ];

        $resp = self::getInstance()
            ->chain('v3/pay/transactions/native')
            ->post(['json' => [
                'mchid'        => self::$config['merchantId'],
